...
function parseAmountString(s) {
    ...
    // اجمع الأرقام (مع الفواصل) ثم اجلب باقي الأحرف كعملة
    const numMatch = str.replace(/\u200F/g,'').match(/-?[0-9\,\.]+/);
    const rawNum = numMatch ? numMatch[0].replace(/,/g,'') : '0';
    const value = parseFloat(rawNum) || 0;
    const currencyPart = str.replace(numMatch ? numMatch[0] : '', '').trim();
    // تطبيع العملة
    let currency = currencyPart;
    if (/\$/.test(currencyPart) || /USD/i.test(currencyPart)) currency = '$';
    else if (/ر\.?س|ريال/i.test(currencyPart)) currency = 'ر.س';
...
function computeTotals() {
    ...
    if (elTotalUSD) elTotalUSD.innerText = '$' + formatNumberForCurrency(totalUSD, '$');
