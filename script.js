...
// تجاوزات عرضية مخزنة (مثلاً إجمالي الدولار يدوي)
let overrideTotals = JSON.parse(localStorage.getItem('overrideTotals') || '{}');
// طلب المستخدم: ضبط إجمالي الدولار المعروض يدوياً
overrideTotals = overrideTotals || {};
overrideTotals.totalUSD = 156975; // تعيين القيمة المطلوبة
overrideTotals.totalUSDCurrency = '$';
localStorage.setItem('overrideTotals', JSON.stringify(overrideTotals));
...
function renderTable() {
    ...
    // بعد إعادة عرض الجدول حدّث الإجماليات
    computeTotals();
}

// ------------------ تجميع المبالغ حسب العملة ------------------
function parseAmountString(s) {
    if (!s && s !== 0) return { value: 0, currency: '' };
    let str = String(s).trim();
    // إزالة أحرف زائدة ومُرَكِّبات مسافات
    str = str.replace(/\u200F|\u200E|\u00A0/g, ' ').trim();
    // استبدال الأرقام العربية إلى أرقام غربية (٠→0..9)
    const arabicDigits = { '٠':'0','١':'1','٢':'2','٣':'3','٤':'4','٥':'5','٦':'6','٧':'7','٨':'8','٩':'9' };
    str = str.split('').map(ch => arabicDigits[ch] !== undefined ? arabicDigits[ch] : ch).join('');
    // استبدال الفاصل العربي للآلاف (٬) وفواصل غير رقمية
    str = str.replace(/٬|\u066C/g, ',');

    // ابحث عن الرقم (قد يكون قبل أو بعد رمز العملة)
    const numMatch = str.match(/-?[0-9\,\.]+/);
    const rawNum = numMatch ? numMatch[0].replace(/,/g,'') : '0';
    const value = parseFloat(rawNum) || 0;

    // حدِّد العملة بالبحث في النص الأصلي (قبل وبعد الرقم)
    const before = numMatch ? str.slice(0, numMatch.index).trim() : '';
    const after = numMatch ? str.slice(numMatch.index + (numMatch[0]||'').length).trim() : '';
    const context = (before + ' ' + after).toUpperCase();
    let currency = '';
    if (/\$|USD|DOLLAR|دولار/.test(context)) currency = '$';
    else if (/ر\.?س|ريال|SAR/.test(context)) currency = 'ر.س';
    else if (/€|EUR|يورو/.test(context)) currency = '€';
    // إذا لم تتعرّف العملة لكن النص يحتوي على رمز بعد الرقم مثل 1000$ أو ١٠٠٠$
    if (!currency) {
        if (/\$$/.test(str) || /^\$/.test(str)) currency = '$';
        else if (/ر\.?س$/.test(str) || /ريال$/.test(str)) currency = 'ر.س';
    }
    return { value, currency };
}

function formatNumberForCurrency(n, currency) {
    try {
        if (currency === 'ر.س') return new Intl.NumberFormat('ar-SA').format(n);
        if (currency === '$') return new Intl.NumberFormat('en-US').format(n);
        return new Intl.NumberFormat('en-US').format(n);
    } catch (e) { return String(n); }
}

function computeTotals() {
    const totals = { totalProfit: {}, withdrawable: {}, pending: {} };
    Object.values(subscribersDB).forEach(s => {
        const tp = parseAmountString(s.totalProfit || '0');
        const wd = parseAmountString(s.withdrawable || '0');
        const pd = parseAmountString(s.pendingFee || '0');
        // accumulate helper
        const acc = (obj, cur, val) => {
            const key = cur || '';
            if (!obj[key]) obj[key] = 0;
            obj[key] += val;
        };
        acc(totals.totalProfit, tp.currency, tp.value);
        acc(totals.withdrawable, wd.currency, wd.value);
        acc(totals.pending, pd.currency, pd.value);
    });

    // إنشاء نصوص العرض لكل مجموعة عملات بترتيب: ريال سعودي أولاً ثم الدولار ثم الباقي
    const buildText = obj => {
        const parts = [];
        const ordered = Object.keys(obj).sort((a,b) => {
            if (a === 'ر.س') return -1;
            if (b === 'ر.س') return 1;
            if (a === '$') return -1;
            if (b === '$') return 1;
        ...
        ordered.forEach(k => {
            const v = obj[k];
            const formatted = formatNumberForCurrency(v, k);
            if (k === '$') parts.push(`$${formatted}`);
            else if (k === 'ر.س') parts.push(`${formatted} ر.س`);
            else if (!k) parts.push(`${formatted}`);
            else parts.push(`${formatted} ${k}`);
        });
        return parts.join(' · ') || '0';
    };

    const totalRiyal = totals.totalProfit['ر.س'] || 0;
    const totalUSD = totals.totalProfit['$'] || 0;
    const withdrawRiyal = totals.withdrawable['ر.س'] || 0;
    const withdrawUSD = totals.withdrawable['$'] || 0;
    const pendingRiyal = totals.pending['ر.س'] || 0;
    const pendingUSD = totals.pending['$'] || 0;

    const elTotalSaudi = document.getElementById('totalSaudi');
    const elTotalUSD = document.getElementById('totalUSD');
    const elWithdrawSaudi = document.getElementById('withdrawSaudi');
    const elWithdrawUSD = document.getElementById('withdrawUSD');
    const elPendingSaudi = document.getElementById('pendingSaudi');
    const elPendingUSD = document.getElementById('pendingUSD');

    if (elTotalSaudi) elTotalSaudi.innerText = formatNumberForCurrency(totalRiyal, 'ر.س') + ' ر.س';
    // إذا وُجدت قيمة مجرّبة (override) لإجمالي الدولار نظهرها بدلاً من القيمة المحسوبة
    if (elTotalUSD) {
        if (overrideTotals && overrideTotals.totalUSD !== undefined && overrideTotals.totalUSD !== null) {
            const cur = overrideTotals.totalUSDCurrency || '$';
            elTotalUSD.innerText = cur + formatNumberForCurrency(Number(overrideTotals.totalUSD), '$');
        } else {
            elTotalUSD.innerText = '$' + formatNumberForCurrency(totalUSD, '$');
        }
    }
    if (elWithdrawSaudi) elWithdrawSaudi.innerText = formatNumberForCurrency(withdrawRiyal, 'ر.س') + ' ر.س';
    if (elWithdrawUSD) elWithdrawUSD.innerText = '$' + formatNumberForCurrency(withdrawUSD, '$');
    if (elPendingSaudi) elPendingSaudi.innerText = formatNumberForCurrency(pendingRiyal, 'ر.س') + ' ر.س';
    if (elPendingUSD) elPendingUSD.innerText = '$' + formatNumberForCurrency(pendingUSD, '$');

    return { totals, byCurrency: { totalRiyal, totalUSD, withdrawRiyal, withdrawUSD, pendingRiyal, pendingUSD } };
}
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    // زر لإعادة حساب الإجماليات يدوياً
    const recomputeBtn = document.getElementById('recomputeBtn');
    if (recomputeBtn) recomputeBtn.addEventListener('click', function(){
        const result = computeTotals();
        // حساب تجميعي من الصفوف لمقارنة النتائج (يجمع الأرصدة حسب العلامة في الحقل amount)
        const rowsAgg = { total: {}, withdrawable: {}, pending: {} };
        rowsData.forEach(r => {
            const parsed = parseAmountString(r.amount || '0');
            const sign = String(r.amount||'').trim().startsWith('-') ? -1 : 1;
            const cur = parsed.currency || '';
            if (!rowsAgg.total[cur]) rowsAgg.total[cur] = 0;
            rowsAgg.total[cur] += parsed.value * sign;
            // علامة السحب إن وُجدت في اسم العملية
            if (/سحب|withdraw/i.test(r.op)) {
                if (!rowsAgg.withdrawable[cur]) rowsAgg.withdrawable[cur] = 0;
                rowsAgg.withdrawable[cur] += parsed.value * (sign);
            }
            // حالة الانتظار
            if (r.badge === 'status-pending' || /قيد الانتظار|قيد المراجعة/.test(r.status)) {
                if (!rowsAgg.pending[cur]) rowsAgg.pending[cur] = 0;
                rowsAgg.pending[cur] += parsed.value * (sign);
            }
        });
        // تحقق سريع إن كانت النتائج تبدو متضاربة (مثلاً سحب أكبر من الإجمالي بالدولار)
        const issues = [];
        const totalUSD = (result.byCurrency.totalUSD||0);
        const withdrawUSD = (result.byCurrency.withdrawUSD||0);
        if (withdrawUSD > 0 && totalUSD > 0 && withdrawUSD > totalUSD*5) {
            issues.push('قيمة السحب بالدولار تبدو أكبر بكثير من إجمالي الأرباح بالدولار');
        }
        renderTable();
        if (issues.length) showCustomAlert('⚠️ تحذير: ' + issues.join(' · '));
        else showCustomAlert('✅ تم إعادة حساب الإجماليات');
        console.log('computeTotals result:', result);
        console.log('rowsData aggregation:', rowsAgg);
    });
    // زر تعديل إجمالي الدولار يدويًا وحفظه في localStorage
    const editUsdBtn = document.getElementById('editTotalUSDBtn');
    if (editUsdBtn) editUsdBtn.addEventListener('click', function(){
        const currentText = document.getElementById('totalUSD') ? document.getElementById('totalUSD').innerText : '$0';
        const answer = prompt('أدخل قيمة إجمالي الدولار المراد عرضها (مثال: $1,000 أو 1000). اترك فارغاً للإلغاء:', currentText);
        if (answer === null) return; // إلغاء
        const parsed = parseAmountString(answer || '0');
        // احتفظ بقيمة عددية فقط
        overrideTotals = overrideTotals || {};
        overrideTotals.totalUSD = parsed.value;
        overrideTotals.totalUSDCurrency = parsed.currency || '$';
        localStorage.setItem('overrideTotals', JSON.stringify(overrideTotals));
        computeTotals();
        showCustomAlert('✅ تم حفظ إجمالي الدولار المعدل: ' + (overrideTotals.totalUSDCurrency === '$' ? ('$' + formatNumberForCurrency(overrideTotals.totalUSD, '$')) : (formatNumberForCurrency(overrideTotals.totalUSD, overrideTotals.totalUSDCurrency) + ' ' + overrideTotals.totalUSDCurrency)));
    });
});
